...
void EducationLuncherClient::slotDisconnected()
{
   ...
   // disconnect(tcpClient,SIGNAL(readyRead()),this,SLOT(slotreadServer()));
...
void EducationLuncherClient::slotreadServer() //当服务端有新信息时触发更新UI界面
{
     ...
     // ui->label->setText(serverMsg);
      int switchMessage=serverMsg.toInt();

      switch (switchMessage) {
      case 1: ui->label->setText(serverMsg);  //确认连接
          break;
      case 2:slotOpenProject();  //打开工程
          break;


      default: ui->label->setText(serverMsg);
...
void EducationLuncherClient::slotOpenProject()
{
